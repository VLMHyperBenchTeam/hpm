...
Для поддержки слияния в манифестах контейнеров используются плейсхолдеры вида `${HSM_MERGED_PARAMS.key}`.

**Пример: Один Postgres — много баз данных**
1.  **Пакет A** (в реестре) подразумевает `container:postgres` с параметрами `db_name: "vector_db"`.
2.  **Пакет B** подразумевает `container:postgres` с параметрами `db_name: "graph_db"`.
3.  **Манифест Postgres** (в реестре) содержит:
    ```yaml
    env:
      POSTGRES_MULTIPLE_DATABASES: "${HSM_MERGED_PARAMS.db_name}"
    ```
4.  **HSM Sync Engine** объединяет значения:
    *   Генерируется **один** сервис `postgres`.
    *   Переменная окружения принимает вид: `POSTGRES_MULTIPLE_DATABASES=vector_db,graph_db`.

### Алгоритм слияния
1.  **Collection**: `SyncEngine` обходит все выбранные пакеты и группы, собирая секции `implies`.
2.  **Grouping**: Параметры группируются по `target_id` (например, `container:postgres`).
3.  **Deduplication**: Значения параметров внутри одной группы дедуплицируются.
4.  **Injection**: При генерации конфигурации контейнера, `SyncEngine` ищет плейсхолдеры `${HSM_MERGED_PARAMS.*}` и заменяет их на списки значений, разделенные запятой.
