# ADR 003: Манифест hpm.yaml и архитектура адаптеров
...
### 1. Манифест `hpm.yaml`
Файл `hpm.yaml` становится основным источником правды. Для его редактирования используется библиотека `ruamel.yaml`, что позволяет сохранять комментарии и форматирование пользователя.
...
*   **DockerAdapter**: Использует механизм **Profiles** (Docker Compose v2) для активации групп сервисов и генерирует `docker-compose.hpm.override.yml`.
...
### 5. CLI Структура
*   **Project**: `hpm init`, `hpm sync`, `hpm check`, `hpm list`.
*   **Edit**: `hpm add/remove` (поддерживает работу с пакетами, контейнерами и группами).
*   **Registry**: `hpm registry ...` (справочная информация).
