...
1.  **Команда**: `hsm add group vector-db --option milvus`.
2.  **Действие HSM**:
    *   В `pyproject.toml` прописывается клиент для Milvus.
    *   В `docker-compose.hpm.yml` прописывается запуск контейнера Milvus вместо Qdrant.
...
| :--- | :--- | :--- |
| **Инициализация** | `hsm init` | Создание `hsm.yaml` на основе `pyproject.toml`. |
| **Добавление** | `hsm add <name>` | Поиск в Реестре и добавление в манифест. |
| **Разработка** | `hsm mode <name> dev` | Переключение компонента на локальные исходники. |
| **Синхронизация** | `hsm sync` | **Атомарное** обновление конфигов `uv` и `docker`. |
| **Запуск** | `hsm run` | Выполнение кода в подготовленном окружении. |
