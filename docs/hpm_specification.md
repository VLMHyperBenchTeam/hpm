...
*   **Docker Adapter**: Управляет сервисами через генерацию `docker-compose.hsm.yml` и использование профилей.
...
### 3.1. Components
1.  **Registry Manager**: Справочник доступных компонентов, разделенный на `packages`, `containers` и `groups`.
2.  **Manifest Engine**: Управляет файлом `hsm.yaml`, обеспечивая атомарность изменений и сохранение комментариев (Round-Trip).
3.  **Adapter Layer**: Транслирует намерения из манифеста в конфигурации для `uv`, `docker compose` и других инструментов.
