...
from ..models import PackageManifest, ContainerManifest
...
class SyncEngine:
    ...
    def sync(self, frozen: bool = False):
        ...
        for pkg_name in self.manifest.packages:
            ...
            if pkg_req:
                packages_to_sync[pkg_name] = pkg_req
                # Collect implies from standalone package
                pkg_manifest_path = self.registry_path / "packages" / f"{pkg_name}.yaml"
    ...
    def _resolve_package_requirement(self, name: str) -> Optional[str]:
        """Resolve a package name to a requirement string."""
